...
while ($hugp= mysqli_fetch_array($hup))
{
  ...
  $timeOutsecondsp = $hugp["timeOutseconds"];

  $hourdiffp += (abs(($timeInsecondsp / 1000) - ($timeOutsecondsp / 1000)) / 60);
}
...
while ($hugpx = mysqli_fetch_array($hupx))
{
  ...
  $timeOutx = date("Y-m-d h:i:s A", ($timeInsecondspx / 1000));

  $hourdiffpx = round((abs(($timeInsecondspx / 1000) - ($timeOutsecondspx / 1000)) / 3600) / 60, 2);
