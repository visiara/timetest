...
if (!isset($uid) || !isset($pwd) || empty($uid) || empty($pwd)) {
  ...
  $accesssql = "SELECT * FROM employee WHERE uname = '$uid' AND pword = '$pwd' AND status = 'Active'";
  ...
  if (mysqli_num_rows($accessresult) == 0) {
    ...
    $upllogin = mysqli_query($conn, "UPDATE employee SET llogin = '$llogin2' WHERE uname = '$uid' AND pword = '$pwd'");
