...
if (isset($_GET['pid'])) {
  $ppid = $_GET['pid'];
  $saveinsert1 = "UPDATE attendance SET aover = '1', overby = '$uid' WHERE id='$ppid'";
  if (mysqli_query($conn, $saveinsert1)) {

    //log event
    log_audit_event($uid, "UPDATE", "Updated record: $ppid", "Clocked out User", "success", '', '', $_SESSION['logged']);

    $status = '<div class="alert alert-success" role="alert">
        <strong>Well done!</strong> You successfully Clocked-Out the User.
    </div>';
  } else {
...
<head>
  ...
  <script langauge="javascript">
    function reload(form) {
      ...
      self.location = 'attendance_overrule.php?date=' + month_val; // reload the page
    }
  </script>
          ...
          <table id="datatable1" class="table table-bordered display responsive ">
            ...
            <tbody>
              ...
              while ($huserb1d5 = mysqli_fetch_array($huserbd5)) {
                ...
                $timeOutseconds = '999999999999999999';
                $timeStatus2 = 'off';
                $timeOutsecondsMain = '999999999999999999';
              ...
              ?>
                  <tr>
                    ...
                    <td><?php echo $timeOut; ?>
                      ...
                      if ($timeOutsecondsMain == "0" && $timeInseconds != "0") {
                        if ($aover == "1") {
                          echo "<i class='fa fa-clock tx-success tx-20'></i>";
                        } else {
                          echo "<i class='fa fa-clock tx-danger tx-20'></i>";
                        }
                      }
                      ?>
                    </td>
                    <td>
                      <?php if ($aover == "0") { ?>
                        <a href="?pid=<?php echo $did; ?>&date=<?php echo $ddate; ?>" class="btn btn-primary btn-sm" onclick="return confirm('Are you sure you wish to Clock-Out this User?');">Clock Out</a>
                      <?php } ?>
                    </td>
                  </tr>
              <?php
